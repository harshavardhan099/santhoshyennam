...
exports.getRecommendedMovies = async (req, res) => {
  try {
    ...
    let recommendations = [];
    let flaskApiUrl = "http://127.0.0.1:5000/recommendations"
    if (user_id) {
      // Make API call to Flask server with user ID
      flaskApiUrl = `http://127.0.0.1:5000/recommendations?user_id=${user_id}`;
    }
    // Using axios to make the HTTP GET request
    const response = await axios.get(flaskApiUrl);
    
    // Extracting recommendations from the Flask server response
